...
const list = async ctx => {
  console.time(`${allMonth.length}个月的天气数据抓取并存库时间：`)
  cluster.setupMaster({
    exec:'worker.js',
    args:['--use','http']
  })
  let allMonthLength=allMonth.length
  for(let i=0;i<cpuNums;++i){
    let work = cluster.fork();
    work.send([i,cpuNums,allMonth])
    work.on('message',async msg=>{
      ...
      if(allMonthLength==0){
        console.log('爬取完毕，开始关闭所有子进程')
        console.log('爬取出错月份:'+errMonth)
        console.timeEnd(`${allMonth.length}个月的天气数据抓取并存库时间：`)
        cluster.disconnect()
      }
    })
  }
